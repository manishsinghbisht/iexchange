...
def json_extract(batch_count, file_name, file_output_location, file_type, obj_extract_request, obj_file):
    if (file_type ==  'JSON' or app_level.appConfig['DEFAULT']['WRITE_FOUTPUT_JSON'] == "TRUE"):
        ...
        with open(finaloutputdata_file_name, 'w') as outfile:
            #json.dump(app_level.massagedJsonData, outfile, sort_keys=True, indent=4) 
        ...
        try:
            ...
            if(document):
                ...
                if(cursor_document_count % file_data_chunk_size == 0 or cursor_document_count >= total_documents_count):
                    ...
                    elif(app_level.appConfig['EXTRACT_FILES']['WRITE_CSV'] == True):
                        logger.info("Starting to writing csv")   
                        csv_writer.write_extract_csv_file(batch_count = batch_count, file_output_location=file_output_location,file_name=file_name, col_names = col_names, delimiter=file_delimiter, shall_quote=quote_all)
                        logger.info("Completed writing csv") 

                    #if(app_level.appConfig['EXTRACT_FILES']['WRITE_FWF'] == True):
                    #    fwf_writer.write_extract_fwf_file()
