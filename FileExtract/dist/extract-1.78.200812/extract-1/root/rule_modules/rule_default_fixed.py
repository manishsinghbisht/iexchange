 ...
 # Constructor
    ...
    def executeRule(self):
        ...
        if rule['InputColumnType'] != "JSON":
            logger.error("DefaultFixed rule is only available for InputColumnType = JSON. {0}".format(rule['RuleKey']))
            return
        
        if rule['OutputColumnHead'] == None or rule['OutputColumnHead'] == '':
            logger.error("OutputColumnHead required. Rule {0}".format(rule['RuleKey']))
            return
