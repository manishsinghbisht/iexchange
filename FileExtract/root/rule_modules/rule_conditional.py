...
class rule_conditional(object):
 ...
 # Constructor
    ...
    def executeRule(self):
        ...
        if rule['RuleKey'] == "RuleKey_Conditional_Default_NoInputColumn" or rule['RuleKey'] == "RuleKey_Conditional_InputColumn":
            logger.info("Test case hit")
        ...
        for r in app_level.massagedJsonData:
            ...
            if rule["InputColumnType"] == "DB":
                ...
                c1 = Assign.Assign_Input_To_Draft(rule, record, input_column_head = rule['ConditionInputColumnHead'], 
                                                  input_column = rule['ConditionInputColumn'], draft_value = '', 
                                                  input_filter_column = rule['Condition_Input_Filter_Column'], 
                                                  input_filter_value = rule['Condition_Input_Filter_Value'], 
                                                  input_filter_index = int(rule['Condition_Input_Filter_Index']))
