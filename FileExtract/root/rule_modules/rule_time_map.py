    ...
    def executeRule(self):
        ...
        for r in app_level.massagedJsonData:
            try:
                ...
                #if(draftValue != ''):
                #    logger.info('Time map debug!')

                try:
                    # Check the time format
                    time_value=datetime.strptime(draftValue, '%I:%M %p').time()
                    time_format = rule['time_format']
                    if(time_format == '12 hour'):
                        draftValue = time_value.strftime('%I:%M:%S %p')
                    elif(time_format == '24 hour'):
                        draftValue = time_value.strftime('%H:%M:%S')
                    elif(time_format == 'Military'):
                        draftValue = time_value.strftime('%H%M')
                except ValueError:
                    logger.error("Incorrect time string format. It should be 'HH:MM AM/PM' Ex- '01:30 PM' Rule: {0} Record: {1}".format(self.Rule['RuleKey'], record['IX_REC_UID']))
