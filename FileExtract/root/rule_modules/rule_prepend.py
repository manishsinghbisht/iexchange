...
class rule_prepend(object):
    """description of class"""

    # Class level attribute
    RuleType = "StringPrepend"
    Rule = None
    ...
    def executeRule(self):
        ...
        for r in app_level.massagedJsonData:
            ...
            try:
                ...
                draftValue = str(Assign.Assign_Input_To_Draft(rule, record))
                stringToPrepend = rule['StringToPrepend']
                stringLength = int(rule['StringLength'])
                try:
                    draftValue = str(stringToPrepend) + draftValue
                    current_string_length = len(draftValue)
                    #This loop is to make string of certain length by prepending
                    if stringLength > 0:
                        while current_string_length < stringLength:
                            draftValue = str(stringToPrepend) + draftValue
                            current_string_length = len(draftValue)
                except:
