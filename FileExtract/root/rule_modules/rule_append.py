...
class rule_append(object):
    """description of class"""

    # Class level attribute
    RuleType = "StringAppend"
    Rule = None
    ...
    def executeRule(self):
        ...
        for r in app_level.massagedJsonData:
            ...
            try:
                ...
                stringToAppend = rule['StringToAppend']
                stringLength = int(rule['StringLength'])
                try:
                    draftValue += str(stringToAppend)
                    current_string_length = len(draftValue)
                    #This loop is to make string of certain length by appending
                    if stringLength > 0:
