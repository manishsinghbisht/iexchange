...
class mainExecuteModule(object):
    ...
    codername = "Manish"
    ruleTypeList = ["IXTimeStamp", "ColumnMap", "Rule_NPI", "Rule_SSN", "Merge", "Split", "StringRemove", "Conditional", "StringAppend", "StringPrepend", "StringLowerUpper", "StringReplace", "PickChar", "RemoveColumn"]
    ruleNameList = ["IXTimeStamp", "Rule_NPI", "Rule_SSN", "Merge", "Split"]
    mongoDbFileMappingObj = None

    # Constructor
    def __init__(self, importRequestId, sourceId, fileId, fileName, fileType):
    ...
    def cleanUp(self):
        try:
            ...
            logger.debug("Exception: cleanUp failed.\n{0}\n{1} \n".format(e, e.args))
            logger.exception(e)
