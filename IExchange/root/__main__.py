...
def Process_Queued_Import_Requests(input_filename, input_filetype, input_import_request_id, inputSourceSetUpId, input_fileId, input_delimiter, input_colNames, input_colWidths, mongoObj):
    ...
    try:
        ...
        for x in importRequests:
            ...
            x["Files"].sort(key=get_files_sort_key)
            for f in x["Files"]:
                if f["FileRequestStatus"] == "Queued":              
                    ...
                    if os.path.isfile(file_path_name):
                        ...
                        logger.info("Found File: {0} -- FileId: {1} -- Name: {2}".format(file_path_name, f["Id"], f["Name"]))
                        ...
                        if "IsHeader" in f.keys():
                            isHeader = f["IsHeader"]
                        else:
                            isHeader = None


                        logger.info("Processing FileName: {0}, FileType: {1}, FileId: {2} FileRequestStatus: {3} ".format(f["Name"], f["TYPE"], f["Id"], f["FileRequestStatus"]))
                        ...
                        mainFileProcessObj = main_file_process.mainFileProcess(importRequestId = input_import_request_id, sourceId=inputSourceSetUpId, input_filename = input_filename, input_filetype = input_filetype, input_fileId = input_fileId, input_file_has_header = isHeader, input_delimiter = input_delimiter, col_Names = input_colNames, col_Widths = input_colWidths, created_by_id = createdById, tenant_id = tenantId, sub_tenant_id = subTenantId)
                        mainFileProcessObj.process_inputfile_in_chunks()
                    else:
                        logger.error("File not found exception: Import Request_id {0}. \n File Id and name: {1} ({2}) \n Error: {3}".format(input_import_request_id, f["Id"], f["Name"], "File not found!"))
...
def Process_ConfigDefined_Import_Files(input_filename, input_filetype, input_import_request_id, inputSourceSetUpId, input_fileId, input_delimiter, input_colNames, input_colWidths, mongoObj):
    ...
    #for input_file in input_file_list:
         try:
            ...
            # Start File Processing
            mainFileProcessObj = main_file_process.mainFileProcess(importRequestId = "", sourceId="", input_filename = input_file, input_filetype = input_filetype, input_fileId = input_file + "_" + input_filetype, input_file_has_header = False, input_delimiter = input_delimiter, col_Names = input_colNames, col_Widths = input_colWidths, created_by_id = '9f5040c2-4c81-e911-8531-1c1b0db9f347', tenant_id = '9f5040c2-4c81-e911-8531-1c1b0db9f347', sub_tenant_id = '9f5040c2-4c81-e911-8531-1c1b0db9f347')
            mainFileProcessObj.process_inputfile_in_chunks()
         except Exception as e:
