...
class mainFileProcess(object):
    ...
    #logger = logging.getLogger()
     ...
     # Constructor
    def __init__(self, importRequestId, sourceId, input_filename, input_filetype, input_fileId = None, input_file_has_header = None, input_delimiter = None, col_Names = None, col_Widths = None, created_by_id = None, tenant_id = None, sub_tenant_id = None):
        self.importRequestId = importRequestId
        self.sourceId = sourceId
        self.input_filename = input_filename
        self.input_filetype = input_filetype
        self.input_file_has_header = input_file_has_header
    ...
    def __execute(self, inputJsonData):
        ...
        try:
            ...
            for r in appLevel.massagedJsonData:
                ...
                record["CreatedBy"] = self.created_by_id
                record["TenantId"] = self.tenant_id
                record["SubTenantId"] = self.sub_tenant_id
            ...
            mainExecuteObj = main_execute.mainExecuteModule(importRequestId = self.importRequestId, sourceId = self.sourceId, fileId = self.input_fileId, fileName = self.input_filename, fileType = self.input_filetype, created_by_id = self.created_by_id, tenant_id = self.tenant_id, sub_tenant_id = self.sub_tenant_id)
            ...
            if(self.importRequestId != ""):
                self.mongoObj.update_IX_Import_FileRequest(self.importRequestId, self.input_fileId, "Error", "File Execution error: " + e)
                return "ERROR"
    ...
    def process_inputfile_in_chunks(self):
        try:
            ...
            if self.input_file_has_header:
                file_header = 0
            else:
                file_header = None

            # raise Exception("Sorry, raising exception to show that file movement to error folder works!")
            
            if self.input_filetype.upper() == "CSV" or self.input_filetype.upper() == "TXT":
                ...
                for chunk in pandas.read_csv(rawInputfileNamewithPath, chunksize=chunksize, sep = delimiter, header = file_header, index_col = False, encoding = 'unicode_escape', names = my_input_colNames, 
            ...
            logger.exception(e)
            self.mongoObj.update_IX_Import_FileRequest(self.importRequestId, self.input_fileId, "Error", "File format or read error: " + str(e))
            if(self.importRequestId != ""):
                new_file_name = os.path.splitext(self.input_filename)[0]
                new_file_ext = os.path.splitext(self.input_filename)[1]
                new_file_timestamp = datetime.datetime.now().strftime("%y%m%d_%H_%M_%S")
                new_file_name_with_timestamp = new_file_name + '_' + new_file_timestamp + new_file_ext
                shutil.move(appLevel.appConfig['InputFileLocation'] + "\\" + self.input_filename, appLevel.appConfig['ErrorFileLocation'] + "\\" + new_file_name_with_timestamp)
            return "ERROR"
    ...
    def __processFileLevelTasks(self):
        ...
        if(self.importRequestId != ""):
            ...
            new_file_timestamp = datetime.datetime.now().strftime("%y%m%d_%H_%M_%S")
            new_file_name_with_timestamp = new_file_name + '_' + new_file_timestamp + new_file_ext
            shutil.move(appLevel.appConfig['InputFileLocation'] + "\\" + self.input_filename, appLevel.appConfig['ArchiveFileLocation'] + "\\" + new_file_name_with_timestamp)
